# Use the official Python 3.12 slim version based on Debian Bookworm as the base image
FROM python:3.12-slim

# Copy the 'uv' and 'uvx' executables from the latest uv image into /bin/ in this image
# 'uv' is a fast Python package installer and environment manager
COPY --from=ghcr.io/astral-sh/uv:latest /uv /uvx /bin/

# Set the working directory inside the container to /code
# All subsequent commands will be run from here
WORKDIR /code

# Add the virtual environment's bin directory to the PATH so Python tools work globally
ENV PATH="/code/.venv/bin:$PATH"

# Copy the project configuration files into the container
# pyproject.toml     → project metadata and dependencies
# uv.lock            → locked dependency versions (for reproducibility)
# .python-version    → Python version specification
COPY "pyproject.toml" "uv.lock" ".python-version" ./

# Install dependencies exactly as locked in uv.lock, without updating them
RUN uv sync --locked

# Copy application code into the container
COPY "src/predict.py" "./src/predict.py"

# Copy trained model into the container
COPY "models/model.bin" "./models/"

# Expose TCP port 8000 so it can be accessed from outside the container
EXPOSE 8000

# Run the FastAPI application using uvicorn (ASGI server)
# src.predict:app → refers to 'app' object inside src/predict.py
# --host 0.0.0.0  → listen on all interfaces
# --port 8000     → listen on port 8000
ENTRYPOINT ["uvicorn", "src.predict:app", "--host", "0.0.0.0", "--port", "8000"]